...
def get_model_path():
    """Get the path to the Whisper model, works in both dev and exe environments"""
    if hasattr(sys, '_MEIPASS'):
        # Running as PyInstaller executable
        model_dir = os.path.join(sys._MEIPASS, 'whisper_cache')
        if os.path.exists(model_dir):
            model_files = [f for f in os.listdir(model_dir) if f.endswith('.pt')]
            if model_files:
                # Copy model to temp location where whisper expects it
                temp_whisper_dir = os.path.join(tempfile.gettempdir(), 'whisper_models')
                os.makedirs(temp_whisper_dir, exist_ok=True)
                model_file = model_files[0]
                temp_model_path = os.path.join(temp_whisper_dir, model_file)
                if not os.path.exists(temp_model_path):
                    shutil.copy2(os.path.join(model_dir, model_file), temp_model_path)
                
                # Set whisper cache environment
                os.environ['WHISPER_CACHE_DIR'] = temp_whisper_dir
                return temp_whisper_dir
    
    # Running in development - use default cache
    return None
...
class TranscriptionWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Setup model path for executable
            get_model_path()
            ...
            if self.model is None:
                try:
                    ...
                    self.transcription_ready.emit(f"Error loading Whisper model: {str(e)}. Try running the app normally first to download the model.")
                    return
